  ...
  return (
    <main className="card space-y-3">
      ...
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <input className="rounded border p-2 md:col-span-2" placeholder="内容" value={content} onChange={(e) => setContent(e.target.value)} />
      </div>
      ...
      <ul className="space-y-2">
        {state.diaries.filter((d) => d.date === date).map((d) => (
          <li key={d.id} className="rounded bg-[#f7efe5] p-2">
            ...
            <textarea className="mb-2 h-20 w-full rounded border p-2" value={d.content} onChange={(e) => updateDiary(d.id, { content: e.target.value })} />
